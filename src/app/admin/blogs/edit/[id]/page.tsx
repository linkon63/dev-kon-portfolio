...
import { listCollection, updateItem } from "@/lib/collections";
...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    listCollection<Blog>(COLLECTIONS.blogs)
      .then((blogs) => {
        ...
        // Parse sections from content JSON or legacy plain text split
  ...
  const save = async () => {
    ...
    try {
      ...
      await updateItem(COLLECTIONS.blogs, id, data);
    ...
    return (
      <AdminShell title="Edit Blog">
        <div className="flex h-64 items-center justify-center gap-2 text-sm text-neutral-500">
          <Loader2 className="animate-spin" size={16} /> Loading blog post data...
        </div>
      </AdminShell>
    );
  }
              ...
              <div className="mt-16 border-t border-neutral-200 pt-10 opacity-50 select-none">
                {draft.allowLikes && (
                  <p className="text-xs text-neutral-400 font-semibold mb-4">
                    ❤️ Likes enabled (Preview only) — Current count: {draft.likes || 0}
                  </p>
                )}
                {draft.allowComments && (
